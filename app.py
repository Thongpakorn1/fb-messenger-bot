...
def analyze_image_with_gpt4(image_url):
    ...
    product_descriptions = "\n".join([
        f"{item['name']} - ขนาด: {item.get('size', 'ไม่ระบุ')}, น้ำหนัก: {item.get('weight', 'ไม่ระบุ')} กรัม, ราคา: {item.get('price', 'ไม่ระบุ')} บาท"
        for item in product_list
    ])

    prompt_text = f"""
    คุณคือนักวิเคราะห์ภาพสินค้าโบราณที่มีความแม่นยำสูง
    จากภาพสินค้านี้ ช่วยวิเคราะห์ **ขนาด, น้ำหนัก, ราคา** ของสินค้าจากลักษณะรูปร่าง, การออกแบบ และวัสดุที่ใช้
    โดยพิจารณาจากข้อมูลสินค้าด้านล่าง:
    {product_descriptions}
    """  # ลดข้อมูลที่ไม่จำเป็นออก
