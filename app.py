...
# ฟังก์ชันที่ใช้ในการรับข้อมูลรูปภาพจาก Telegram
def process_image_from_telegram(image_url, sender_id):
    print(f"📷 ลูกค้าส่งภาพ: {image_url}")

    # ส่งข้อความแจ้งเตือนว่าได้รับรูปภาพแล้ว
    send_telegram_notification(f"🚨 ลูกค้าส่งรูปภาพ: {image_url}\nกรุณาตรวจสอบข้อมูลภาพนี้ด้วย")

    # ดำเนินการประมวลผลรูปภาพ
                        ...
                        if "attachments" in messaging_event["message"]:
                            for attachment in messaging_event["message"]["attachments"]:
                                if attachment["type"] == "image":
                                    ...
                                    # ส่งข้อความแจ้งเตือนทาง Telegram ว่ามีการส่งรูปภาพเข้ามา
                                    send_telegram_notification(f"🚨 ลูกค้าส่งรูปภาพมาให้ตรวจสอบ: {image_url}")
                                    
                                    # ส่งภาพไปให้ GPT-4o วิเคราะห์
                                    vision_reply = analyze_image_with_gpt4(image_url)
                                    
                                    # ส่งคำตอบหรือการแจ้งเตือนกลับไปยังลูกค้า
                                    send_message(sender_id, vision_reply)
