...
def analyze_image_with_gpt4(image_url):
    ...
    # สร้าง string ที่มีรายละเอียดสินค้าทั้งหมดจาก product_list
    product_details_prompt = "\n".join([
        f"ชื่อสินค้า: {item['name']}\n"
        f"วัสดุ: {item.get('material', 'ไม่ระบุ')}\n"
        f"ลักษณะการออกแบบ: {item.get('design', 'ไม่ระบุ')}"
        for item in product_list
    ])

    prompt_text = f"""
คุณคือผู้ช่วยวิเคราะห์ภาพสินค้าโบราณที่มีความแม่นยำสูง
จากภาพที่ลูกค้าส่งมานี้ จงวิเคราะห์อย่างละเอียด โดย **มองหารูปทรงโดยรวม, สังเกตลวดลาย, ตรวจสอบอัญมณี (ชนิด, สี, จำนวน), และเปรียบเทียบสีและวัสดุที่ปรากฏในภาพ**

จากนั้น **เปรียบเทียบสิ่งที่พบในภาพกับรายการสินค้าที่มีอยู่ในระบบ** โดยเน้นความสอดคล้องกับ 'ชื่อสินค้า', 'วัสดุ', และ 'ลักษณะการออกแบบ' ที่ระบุไว้

นี่คือรายการสินค้าที่มีอยู่ในระบบ พร้อมรายละเอียด:
{product_details_prompt}

เมื่อวิเคราะห์ภาพแล้ว ให้ระบุสินค้าที่ **ตรงกันมากที่สุด หรือใกล้เคียงที่สุด** เพียงรายการเดียว โดยพิจารณาจากลักษณะที่กล่าวมาข้างต้น **ให้ความสำคัญเป็นพิเศษกับลักษณะเด่น เช่น ลวดลายหลัก, รูปทรงเฉพาะ, และชนิดของอัญมณี**

ถ้าพบสินค้าที่ตรงกันหรือใกล้เคียง ให้ตอบกลับด้วยข้อมูลต่อไปนี้:

ชื่อสินค้า: [ชื่อสินค้าที่พบ]
ขนาด: [ขนาด (ถ้ามี) หรือ 'ไม่ระบุ']
น้ำหนัก: [น้ำหนัก (ถ้ามี) กรัม หรือ 'ไม่ระบุ กรัม']
ราคา: [ราคา (ถ้ามี) บาทค่ะ หรือ 'ไม่ระบุ บาทค่ะ']

โดยดึงข้อมูล 'name', 'size', 'weight', และ 'price' จากรายการสินค้าที่พบ หากไม่พบข้อมูลขนาดหรือน้ำหนัก ให้ระบุว่า 'ไม่ระบุ'
