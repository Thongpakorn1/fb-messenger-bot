...
def analyze_image_with_gpt4(image_url):
    ...
    # รวมรายละเอียดสินค้าในรูปแบบที่ GPT เข้าใจง่าย
    product_details = "\n".join([
        f"ชื่อสินค้า: {item['name']}, วัสดุ: {item.get('material', 'ไม่ระบุ')}, ดีไซน์: {item.get('design', 'ไม่ระบุ')}"
        for item in product_list
    ])

    prompt_text = f"""
คุณคือนักวิเคราะห์ภาพสินค้าโบราณที่มีความแม่นยำสูง
จากภาพที่ลูกค้าส่งมานี้ จงวิเคราะห์และเปรียบเทียบอย่างละเอียดกับรายการสินค้าที่มีอยู่ในระบบ โดยพิจารณาลักษณะต่างๆ ที่ปรากฏในภาพ เช่น ประเภท, วัสดุ, ดีไซน์, ลวดลาย, รูปทรง และรายละเอียดอื่นๆ

นี่คือรายการสินค้าที่มีอยู่ในระบบ พร้อมรายละเอียด (ชื่อสินค้า, วัสดุ, ดีไซน์):
{product_details}

เมื่อวิเคราะห์ภาพแล้ว ให้ระบุสินค้าที่ **ตรงกันมากที่สุด หรือใกล้เคียงที่สุด** เพียงรายการเดียว โดยพิจารณาจากลักษณะที่กล่าวมาข้างต้น

ตอบกลับด้วยรายละเอียดของสินค้านั้นในรูปแบบต่อไปนี้เท่านั้น:
...
โดยดึงข้อมูลจาก fields 'name', 'size', 'weight' (ตัดคำว่า ' กรัม' ออก), และ 'price' (ตัดคำว่า ' บาทค่ะ' ออก) ใน JSON เท่านั้น หากไม่พบข้อมูลขนาดหรือน้ำหนัก ให้ระบุว่า 'ไม่ระบุ'

หากไม่พบสินค้าที่ตรงกันหรือใกล้เคียงในภาพ **หลังจากพิจารณาทุกรายละเอียดแล้ว** ให้ตอบกลับด้วยข้อความ: "ไม่พบสินค้าที่ตรงกันในระบบค่ะ"

**สำคัญ:** ตอบกลับด้วยข้อมูลสินค้าตามรูปแบบที่กำหนดเท่านั้น ห้ามพูดนอกเหนือจากนี้
"""
