...
def analyze_image_with_gpt4(image_url):
    ...
    # รวมรายละเอียดสินค้าพร้อมลักษณะเด่นและการออกแบบ
    product_descriptions = "\n".join([
        f"{item['name']} (ลักษณะเด่น: {item.get('key_features', 'ไม่ระบุ')}, การออกแบบ: {item.get('design', 'ไม่ระบุ')}) - ขนาด: {item.get('size', 'ไม่ระบุ')}, น้ำหนัก: {item.get('weight', 'ไม่ระบุ')} กรัม, ราคา: {item.get('price', 'ไม่ระบุ')} บาท"
        for item in product_list
    ])

    prompt_text = f"""
    คุณคือนักวิเคราะห์ภาพสินค้าโบราณที่มีความแม่นยำสูง
จากภาพด้านล่าง ช่วยวิเคราะห์ **ลักษณะเด่น** ของสินค้าจากลักษณะรูปร่าง, การออกแบบ, และวัสดุที่ใช้ ได้แก่ **ทรง, สี, ลวดลาย, และวัสดุ** อย่างละเอียด จากนั้นเปรียบเทียบกับรายการสินค้าที่มีอยู่ในระบบ โดยพิจารณาจาก **ลักษณะเด่น (key_features)** และ **การออกแบบ (design)** ที่ระบุไว้ และตรวจสอบให้แน่ใจว่า **ข้อมูลขนาด, น้ำหนัก และราคา** ของสินค้าที่ใกล้เคียงที่สุดได้รับการตอบกลับอย่างครบถ้วน หากพบสินค้าที่ตรงกับลักษณะเด่นและการออกแบบให้ตอบกลับตามรูปแบบตัวอย่างนี้เท่านั้น:
