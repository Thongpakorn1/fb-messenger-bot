...
def analyze_image_with_gpt4(image_url):
    ...
    product_details = "\n".join([
        f"ชื่อสินค้า: {item['name']}, ขนาด: {item.get('size', 'ไม่ระบุ')}, น้ำหนัก: {item.get('weight', 'ไม่ระบุ')} กรัม, ราคา: {item.get('price', 'ไม่ระบุ')} บาท"
        for item in product_list
    ])

    prompt_text = f"""
คุณคือนักวิเคราะห์ภาพสินค้าโบราณที่มีความแม่นยำสูง
จากภาพที่ลูกค้าส่งมานี้ จงวิเคราะห์และเปรียบเทียบกับรายการสินค้าที่มีอยู่ในระบบอย่างละเอียด โดยเน้นที่ลักษณะของสินค้า เช่น ประเภท (แหวน, สร้อย, ต่างหู), วัสดุ, การออกแบบ, และรายละเอียดอื่นๆ ที่ปรากฏในภาพ

นี่คือรายการสินค้าที่มีอยู่ในระบบ:
{product_details}
...
**สำคัญ:** ตอบกลับด้วยข้อมูลสินค้าเท่านั้น ห้ามพูดนอกเหนือจากรูปแบบที่กำหนด
"""
